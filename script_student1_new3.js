...
function userDeleter(mainAppObject, userId) {
  ...
  //add
  console.log("Deleter ind=", userDbIndex);
...
function activateForm(mainAppObject, state) {
  const form = mainAppObject.selectors.userForm;
  let s = state;
  form.addEventListener("submit", function (e) {
    paramFunction(e, state);
  });
  function paramFunction(e, st) {
    e.preventDefault();
    ...
    if (st == "Change") {
      let changeUserData = createUserObject(this);
      changeUsers(mainAppObject, changeUserData);
    }
  }
}
...
function changeUsers(mainAppObject, changeUserData) {
  ...
  console.log("change ind", studentNuber);
  const changeUser = changeUserData;
  console.log("edit", changeUser);

  //changeUser.id = userDbIndex; add???????
  changeUser.id = studentNuber;
  console.log("changeStue", changeUser.id);
...
function editUserObject(userForm) {
  console.log("editUser id=", studentNuber);
  let userResult = {
    details: {},
  };
  for (let formInput of userForm) {
...
function userEditer(mainAppObject, userId) {
  const userDatabase = mainAppObject.userDatabase;
  const form = mainAppObject.selectors.userForm;
  console.log("useEditer-state-begin", mainAppObject.formEditState);
  let userDbIndex = userDatabase.indexOf(
    userDatabase.find((userInfo) => {
      return userInfo.id == userId;
    })
  ...
  for (let userItem of userDatabase) {
    if (userItem.id == userDbIndex + 1) {
      //add
      studentNuber = userDbIndex + 1;
      console.log("Editer", studentNuber);
